...
const PublishedPost: React.FC<IProps> = ({ route, navigation }) => {
    const { bookmarks, setAll } = useFeedsState((state) => state)
    const [data, setData] = React.useState<PostModel | null>(null);
    const { params: { id }} = route;
    // const { user: { business_name, logo, id: businessId }, likes, comments, description, images, createdAt, id: postId } = data as PostModel;
    const { isLoading: postLoading } = useQuery(['getPost', id], () => httpClient.get(`/post/${id}`), {
        onSuccess: (data) => {
    ...
    if (postLoading || data === null || data === undefined) {
        return (
            <View>
        ...
        return (
            <View style={Styles.parent}>
                <View flexDirection='row' height={100} alignItems='center'>
                    <Feather onPress={() => navigation.goBack()} name='chevron-left' size={25} />
                    <Text variant='body'>Published Posts</Text>
                </View>
                {/* HEADER */}
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        ...
                        >
                            <View style={{ width: '100%', height: '100%', borderRadius: 15, justifyContent: 'center', alignItems: 'center', backgroundColor: 'white', overflow: 'hidden' }} >
                                <Image source={{ uri: data?.user.logo }} resizeMode='cover' style={{ width: '100%', height: '100%'}} />
                            </View>
                        </LinearGradient>
                        <View style={{ paddingLeft: 10}}>
                            <Text onPress={() => navigation.navigate('view-profile', { id: data?.user.id })} variant='medium'>{data?.user.business_name}<Text variant='body'>@{data?.user.business_name}</Text></Text>
                            <Text variant='xs'>
                                <TimeAgo time={data?.createdAt as string} interval={20000} />
                            </Text>
                        </View>
                    </View>
                ...
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center'}}>
                    ...
                    {!postLoading && (data?.description as string).length > 30 && 
                ...
                <View marginVertical='m' style={{ flex: 1, backgroundColor: 'black', borderRadius: 20, overflow: 'hidden', width: '100%', height:  height / 100 * 60 }}>
                    ...
                    <View style={{ width: '100%', height: 40, backgroundColor: 'transparent', position: 'absolute', bottom: 0, flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
                        {(data?.images as string[]).length > 1 && data?.images.map((item, inx) => (
                ...
                <View style={{ height: 30, flexDirection: 'row', justifyContent: 'space-between'}}>
                    ...
                    <View style={{ flexDirection: 'row' }}>
                            ...
                            }
                            {
                                !isLoading && (
                                    <>
                                        ...
                                        <Text variant='body' style={{ color: '#222222', marginLeft: 4 }}>{data?.likes.length}</Text>
                                    </>
                                )
                            }
                       </Pressable>
                       ...
                       <Pressable onPress={() => navigation.navigate('comments', { postId: id })} style={{ flexDirection: 'row', marginHorizontal: 20, }}>
                            ...
                            <Text variant='body' marginLeft='s' style={{ color: '#222222', marginLeft: 4 }}>{data?.comments.length}</Text>
                       </Pressable>
